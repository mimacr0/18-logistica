...
class HrEmployee(models.Model):
    ...
    def request_dingtalk_employee_data(self, company_id):
        ...
        if not conf_id:
            raise exceptions.UserError(_("请先配置钉钉应用API参数！"))
        try:
            ...
            for dept_id in self.env['hr.department'].search([('company_id', '=', company_id), ('dingtalk_id', '!=', False)]):
                _logger.info(_(f"开始同步从钉钉中同步[{dept_id.name}]的员工数据..."))
    ...
    def update_department_emp_list(self, employee_list, company_id):
        ...
        if not employee_list:
            _logger.warning(_("未收到任何员工数据，跳过同步..."))
            return
        ...
        self.env.cr.commit()
        _logger.info(_(f"钉钉员工同步完成，共更新 {len(updated_emp_ids)} 条，新增 {len(new_emp_values)} 条."))
    ...
    def create_res_users(self, company_id, default_password):
        ...
        if not employees:
            _logger.info(_(f"没有需要创建用户的员工"))
            return
        _logger.info(_(f"发现{len(employees)}名员工需要创建用户"))
        ...
        for emp in employees:
            if emp.mobile_phone in existing_logins:
                _logger.warning(_(f"用户 {emp.mobile_phone} 已存在，跳过"))
                continue
            new_users.append({
                'name': emp.name,
        ...
        if new_users:
            ...
            #     emp.user_id = user.id
            _logger.info(_(f"成功创建 {len(created_users)} 名新用户"))
    ...
    def _auto_sync_dingtalk_hr_cron(self):
        ...
        for company_id in self.env['res.company'].sudo().search([]):
            try:
                ...
                raise exceptions.ValidationError(_(f"同步钉钉部门数据失败: {e}"))
            try:
                ...
                raise exceptions.ValidationError(_(f"同步钉钉员工数据失败：{e}"))
